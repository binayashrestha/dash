...
'''
    Three tabs are created using the dbc.Tabs component: a line chart, a scatter chart, and
    the AG Grid. Each tab contains a Dash Graph component with a Plotly Express figure using
...
'''
The app layout is defined using a Bootstrap Container, Row, and Columns.
The controls and theme controls are placed in one column, while the tabs
and color buttons are placed in another.
'''
...
'''
A callback function is defined to update the line chart, scatter chart, and grid based on
user interactions with the controls. It takes the selected indicator, continents, years,
theme, and color mode switch state as inputs and returns updated figures and grid options.
'''
@callback(
